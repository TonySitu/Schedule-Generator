...
class Course: 
    def __init__(self, crn: str, code: str, section: str, days: list, 
                 start_time: datetime, end_time: datetime, section_tie: str, tutorials = []) -> None:
        self._crn = crn
        self._code = code
        self._days = days
    ...
    def has_day_conflict(self, other_course: Course) -> bool:
        common_days = set(self._days).intersection(other_course._days)
        return len(common_days) > 0
    
    def __has_time_conflict(self, other_course: Course) -> bool:
        return (self._start_time >= other_course._start_time and self._start_time <= other_course._end_time) \
        or other_course._start_time >= self._start_time and other_course._start_time <= self._end_time
